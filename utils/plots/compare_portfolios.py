...
from utils.math import calc_max_drawdown, gmean, normalize
...
def compare_portfolios(
    ...
    draw_growth_chart(data, "Growth of Portfolios")
    ...
    assert len(short_names) == len(list(portfolios.keys())), (
        f"The list of short names is too short. "
    ...
    for i, (name, results) in enumerate(data.items()):
        ...
        yearly_results = results.pct_change(1, freq="YE").dropna()
    ...
    if details:
        max_drawdown, drawdown_start_date, drawdown_end_date = calc_max_drawdown(
            portfolio_results
        )

        risk_reward = pd.DataFrame(index=short_names, columns=["risk", "reward"])
    ...
    for i, (name, results) in enumerate(data.items()):
        ...
        print(
            f"[{short_names[i]}] {name}: ${annual_results.loc[i, 'start']:.2f}->${annual_results.loc[i, 'end']:.2f} "
