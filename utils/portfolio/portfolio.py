...
class Portfolio:
    ...
    def __init__(
        ...
        assert len(distribution.keys()) == len(set(distribution.keys())), (
            "Every ETF must be unique in your portfilio."
        )
        assert sum([d for d in distribution.values()]) <= 100, (
    ...
    def backtest(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        while True:
            for name in asset_names:
                portfolio_values.loc[start_date:end_date, name] = (
                    data.loc[start_date:end_date, name] * assets[name].amount
                )
    ...
    def _sell(self, assets, prices, target: float):
        ...
        sum_value = sum(
            [(prices[name] * asset.amount) for name, asset in assets.items()]
        )
        for name, asset in assets.items():
            value = asset.amount * prices[name]
            percent = (value / sum_value) * 100
